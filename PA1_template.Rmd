...
```{r}
aa <- as.data.frame(tapply(df$steps,df$interval,mean, na.rm=TRUE))
names(aa) <- c("average_5")
#aa <- cbind(time=rownames(aa),aa)
plot3 <- as.ts(as.matrix(aa))
plot.ts(plot3)
```

```{r computetime,echo=FALSE}
maxidx <- which.max(plot3)
idx <- names(aa[maxidx,1])
maxval <- max(plot3)
```
The index `r idx` of 5-minute interval, on average across all the days in the dataset, contains the maximum number `r maxval` of steps
